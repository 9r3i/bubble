...
function bubble(){
  ...
  popCount.classList.add('pop-count');
  popCount.dataset.value=0;
  popCount.dataset.prefix='Score: ';
  document.body.appendChild(popCount);
  var popStart=(new Date).getTime();
  var popTime=document.createElement('div');
  popTime.classList.add('pop-time');
  document.body.appendChild(popTime);
  timeout();
  
  function timeout(){
    var i=(new Date).getTime()-popStart;
    if(i>=0xea60){
      removeAllElements();
      var con=confirm('Your score is '
        +popCount.dataset.value
        +'\n\nRestart?');
      if(con){return bubble();}
      return true;
    }
    setTimeout(()=>{
      popTime.style.width=Math.ceil(i/0xea60*100)+'%';
      return timeout();
    },0x64);
  }
  
  function removeAllElements(){
    var child=[canvas,popTime,popCount];
    for(var i in child){
